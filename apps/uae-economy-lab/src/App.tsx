...
function TradeTable({ result, onSelect }: { result: Result; onSelect: (id: string) => void }) {
  ...
  return <><div className="chart-toolbar"><div className="segmented" aria-label="Trade flow"><button aria-pressed={direction === 'exports'} onClick={() => setDirection('exports')}>Exports</button><button aria-pressed={direction === 'imports'} onClick={() => setDirection('imports')}>Imports</button></div><span className="quiet-label">USD · baseline prices</span></div>
    ...
    })}</tbody></table></div><div className="table-note">National trade totals. Partner-specific policy effects need a multi-region model.</div></>;
}
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {page === 'simulator' && <div className="workbench">
        ...
        <section className="results-area" aria-label="Simulation results" aria-busy={loading || running}>
          ...
          {loading || (!result && running) ? <div className="loading-state"><div className="skeleton metrics-skeleton"/><div className="skeleton chart-skeleton"/><span>Loading UAE accounts…</span></div> : result && dataset && applied ? <div className={running ? 'results-body is-calculating' : 'results-body'}>
            ...
            <div className="results-footnote"><span><Check size={14}/>Equilibrium solved</span><button onClick={() => setPage('learn')}>Simplified UAE model<Info size={14}/></button><span>Historical baseline · {dataset.year}</span></div>
      ...
      {page === 'model' && <section className="model-page">
        ...
        <div className="model-grid"><div className="model-section"><span className="eyebrow">Data</span><h2>Real accounts. Explicit assumptions.</h2>{dataset && <><dl className="metadata-list"><div><dt>Source</dt><dd><a href={dataset.sourceUrl} target="_blank" rel="noreferrer">{dataset.source}<ArrowSquareOut size={14}/></a></dd></div><div><dt>Reference year</dt><dd>{dataset.year}</dd></div><div><dt>Economy</dt><dd>United Arab Emirates</dd></div><div><dt>Coverage</dt><dd>{dataset.sectors.length} national sectors</dd></div><div><dt>Currency</dt><dd>{dataset.currency} · current prices</dd></div></dl><div className="source-notes">{dataset.limitations.map((s, i) => <p key={i}>{s}</p>)}</div></>}</div>
          <div className="model-section"><span className="eyebrow">Model</span><h2>A connected economy</h2><div className="model-flow"><span>Trade & policy</span><ArrowRight size={17}/><span>Production & prices</span><ArrowRight size={17}/><span>Income & demand</span></div><p className="model-description">A simplified, single-country equilibrium model. Businesses use domestic inputs, imports, labor and capital; prices and output adjust together.</p><dl className="metadata-list"><div><dt>Government & investment</dt><dd>Fixed real demand</dd></div><div><dt>Capital</dt><dd>Fixed in each sector</dd></div><div><dt>Labor</dt><dd>Choose fixed workforce or fixed wage</dd></div><div><dt>International markets</dt><dd>Fixed import prices; responsive export demand</dd></div><div><dt>External financing</dt><dd>Adjusts to balance accounts</dd></div></dl><p className="model-description">This is not the standard GTAP model. Results are conditional scenarios, not forecasts. Trade substitution and labor parameters are modeling assumptions.</p></div>
        </div>
        <div className="model-section parameters"><div><span className="eyebrow">Behavioral assumptions</span><h2>Test a different response</h2><p>Assumed values, not estimated from these accounts.</p></div><label>Labor share (%)<input type="number" min="10" max="90" step="5" value={draft.laborShare} onChange={e => { if (Number.isFinite(e.target.valueAsNumber)) edit('laborShare', Math.min(90, Math.max(10, e.target.valueAsNumber))); }}/></label><label>Import substitution<input type="number" min="0" max="8" step="0.5" value={draft.substitution} onChange={e => { if (Number.isFinite(e.target.valueAsNumber)) edit('substitution', Math.min(8, Math.max(0, e.target.valueAsNumber))); }}/></label><label>Export response<input type="number" min="0.5" max="12" step="0.5" value={draft.exportElasticity} onChange={e => { if (Number.isFinite(e.target.valueAsNumber)) edit('exportElasticity', Math.min(12, Math.max(0.5, e.target.valueAsNumber))); }}/></label><button className="secondary-button" onClick={() => { setPage('simulator'); }}>View live results<ArrowRight size={16}/></button></div><div className="model-section diagnostics"><div><span className="eyebrow">Verification</span><h2>Every result must balance</h2></div><div><span>Solver status</span><strong>{result?.diagnostics.converged ? 'Converged' : 'No result'}</strong></div><div><span>Maximum residual</span><strong>{result ? result.diagnostics.residual.toExponential(2) : '—'}</strong></div><div><span>Iterations</span><strong>{result?.diagnostics.iterations ?? '—'}</strong></div></div>
      ...
      <footer className="app-footer"><span>UAE Economy Lab</span><span>Scenario analysis · Not a forecast</span></footer>
    </main>
